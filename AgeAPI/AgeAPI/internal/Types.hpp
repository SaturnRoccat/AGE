...
namespace AgeAPI
{
    ...
    public:
        ...
        NonOwningPtr& operator=(NonOwningPtr<T>&& other) noexcept {
            if (this != &other) {
                mPtr = other.mPtr;
                other.mPtr = nullptr;
            }
    ...
    struct DotIdentifier : public std::string
    {
        // Default constructor
        ...
        ~DotIdentifier() = default;
        // Copy constructor
        DotIdentifier& operator=(const DotIdentifier& other) = default;
        // Move constructor
        DotIdentifier& operator=(DotIdentifier&& other) noexcept = default;

        DotIdentifier(const std::string& key) : std::string(key)
        {
            if (key.empty())
        ...
        bool operator==(const DotIdentifier& other) const { return *this == other; }

        operator const std::string& () { return *this; }
...
{
    template <>
    struct formatter<AgeAPI::DotIdentifier> {
        constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

        template <typename FormatContext>
        auto format(const AgeAPI::DotIdentifier& id, FormatContext& ctx) {
            return format_to(ctx.out(), "{}", id);
        }
    };
    template <>
    struct hash<AgeAPI::DotIdentifier>
    {
        std::size_t operator()(const AgeAPI::DotIdentifier& id) const
        {
